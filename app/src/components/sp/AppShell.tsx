...
export function AppShell({ user, children }: { user: SessionUser; children: ReactNode }) {
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-7xl flex-col lg:flex-row">
      {/* Mobile top bar */}
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-[var(--sp-line)] bg-[var(--sp-bg)]/85 px-4 py-3 backdrop-blur lg:hidden">
        <Link to="/discover">
          <Logo size={26} />
        </Link>
        <Link to="/profile" className="rounded-full">
          <Avatar name={user.displayName} hue={user.avatarHue} size={34} ring />
        </Link>
      </header>

      {/* Desktop sidebar */}
      <aside className="sticky top-0 hidden h-dvh w-60 shrink-0 flex-col border-r border-[var(--sp-line)] px-4 py-6 lg:flex">
        ...
        <nav className="mt-8 flex flex-col gap-1">
          {NAV.map((item) => (
            <Link
              ...
              activeProps={{
                className:
                  "flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold bg-white/[0.06] text-[var(--sp-ink)] shadow-[inset_0_0_0_1px_var(--sp-line-strong)]",
              }}
            >
              {item.icon}
              {item.label}
        ...
        <div className="mt-auto">
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="block truncate text-xs text-[var(--sp-faint)]">@{user.username}</span>
            </span>
          </Link>
          <button
